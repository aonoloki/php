...
function ajout_message($bdd,$uname,$message) { // Fonction d'ajout des nouveaux messages à la base de donnée
	$req = $bdd->prepare("INSERT INTO message(user_name,message,posted_at) VALUES(:user_name,:message,NOW())");
	$req->execute(array("user_name"=>$uname,"message"=>$message));
}

function message($bdd) { // Fonction d'affichage des messages existant dans la base de données
	$req = $bdd->query("SELECT * FROM message ORDER BY posted_at DESC");

	return $req;
}

function expire_message($bdd) { // Suppression des anciens messages au bout de 30 minutes (par défaut)
	$req = $bdd->query("DELETE FROM message WHERE posted_at < DATE_SUB(NOW(), INTERVAL 30 MINUTE)");
